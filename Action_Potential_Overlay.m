...
% 1: APD90, 2: APD50, 3: dV/dt_max, 4: APA (Amplitude)
stats_matrix = zeros(num_models, 4); 
...
for model_id = 0:(num_models - 1)
    ...
    elseif model_id == 2
        ...
        modified_params(g_Kr_index) = modified_params(g_Kr_index) * (1 - 0.725);
        modified_params(C_persist_NaL_index) = baseline_C_persist_NaL * 2.31 * 3; % Matches your calcium script
    elseif model_id == 3
    ...
    stats_matrix(model_id+1, 3) = results.dVdt_max;
    stats_matrix(model_id+1, 4) = results.APA;
...
%% FIGURE 2: Bar Charts for Morphology Stats
...
fprintf('%-20s | %-10s | %-10s | %-10s | %-10s\n', 'Model', 'APD90 (ms)', 'APD50 (ms)', 'dV/dt(V/s)', 'Amp (mV)');
fprintf('----------------------------------------------------------------------------\n');
for i = 1:num_models
